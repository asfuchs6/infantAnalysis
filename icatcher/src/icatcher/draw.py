...
def put_text(img, text, loc=None):
    ...
    cv2.rectangle(img, text_location, (text_location[0] + text_w, text_location[1] + text_h+12), bg_color, -1)

    cv2.putText(img, text, (text_location[0], text_location[1] + text_h + font_scale + 4),
                font,
                font_scale,
                font_color,
                line_type)
...
def put_arrow(img, class_name, face, color=None):
    """
...
def prepare_frame(frame, bbox, emotion=None, show_bbox=True, show_arrow=False, conf=None, class_text=None,
                  rect_color=None, frame_number=None, pic_in_pic=False):
    """
    ...
    :param emotion: predicted emotion at a given timeframe
    ...
    if show_arrow:
        if class_text is not None:
            ...
            if class_text == "right" or class_text == "left":
                frame = put_arrow(frame, class_text, bbox, rect_color)
    if conf and bbox is not None:
        frame = put_text(frame, "{:.02f}".format(conf),
                         loc=(bbox[0], bbox[1] + int(bbox[3] * 1)))
    if pic_in_pic:
        pic_in_pic_size = 100
        if bbox is None:
    ...
    if class_text is not None and emotion is not None:
        frame = put_text(frame,"Direction:"+ class_text + " Emotion:" +emotion)
    elif class_text is not None:
        frame = put_text(frame,"Direction:"+ class_text)
