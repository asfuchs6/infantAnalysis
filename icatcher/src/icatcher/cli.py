...
def predict_from_video(opt):
    ...
    try:
        video_name = str(video_paths[0]).split("/")[-1] #just want the name of the video without the extension
        emotionsData = pd.read_csv("../face_and_emotion_detection/output/" + video_name.split(".")[0] + ".csv")
        emotion_by_frames = emotionsData["emotions"].tolist()
        opt.emotion_by_frames = emotion_by_frames
    except:
        pass
    ...
    for i in range(len(video_paths)):
        ...
        while ret_val:
            ...
            if len(image_sequence) == opt.sliding_window_size:  # we have enough frames for prediction, predict for middle frame
                ...
                if opt.output_video_path:
                    if opt.emotion_by_frames is not None:
                        opt.emotion = emotion_by_frames[frame_count]

                        # cv2 uses BGR for colors
                        rect_color = (0, 255, 0) if opt.emotion == 'happy' else (
                            (0, 0, 255) if opt.emotion == 'unhappy' else (255, 0, 0))
                    else:
                        rect_color = (0, 255, 0)

                    draw.prepare_frame(cur_frame, cur_bbox, opt.emotion, show_arrow=True, rect_color=rect_color,
                ...
                if opt.output_annotation:
                    if opt.output_format == "raw_output" and opt.emotion_by_frames:
                        with open(prediction_output_file, "a", newline="") as f:
                            f.write("{}, {}, {:.02f},{}\n".format(str(frame_count + cursor + 1), class_text, confidences[cursor], str(opt.emotion)))
                    elif opt.output_format == "raw_output":
                        with open(prediction_output_file, "a", newline="") as f:
                            f.write("{}, {}, {:.02f}\n".format(str(frame_count + cursor + 1), class_text, confidences[cursor]))
                    elif opt.output_format == "PrefLookTimestamp" and opt.emotion_by_frames:
                        if class_text != last_class_text:  # Record "event" for change of direction if code has changed
                            ...
                            with open(prediction_output_file, "a", newline="") as f:
                                f.write("{},0,{}, {}\n".format(frame_ms, class_text, str(opt.emotion)))
                        ...
                        if class_text != last_class_text:  # Record "event" for change of direction if code has changed
                            ...
                            last_class_text = class_text
                if opt.emotion_by_frames:
                    logging.info("frame: {}, class: {}, confidence: {:.02f}, cur_fps: {:.02f}, emotion: {}".format(str(frame_count + cursor + 1), class_text, confidences[cursor], cur_fps(), str(opt.emotion)))
                else:
